...
/* -----------------------------------------------------------------------------
    ...
    Write the code that will ensure the variable "differans" contains the difference of all 
    the numbers in the list (i.e., tal1 - tal2 - tal3 - tal4 - ... and so on).
*/
console.log("Task: C");

const andreTall = [6, 7, 8, 9];

let sumAndreTall = andreTall[0];

for (let i = 1; i < andreTall.length; i++) {
  sumAndreTall -= andreTall[i];
}

console.log(sumAndreTall);
/* -----------------------------------------------------------------------------
    Task: D
...
function differansier(liste) {
  let sumAndreTall = liste[0];

  for (let i = 1; i < liste.length; i++) {
    sumAndreTall -= liste[i];
  }
...
function multipliser(liste) {
  let magicNumberBeGone = 1;
  let sum = magicNumberBeGone;
...
let multiplied = multipliser(tall);
